...
/* Action Potential Duration and Max. Info */
	...
	if(var.out_data_plus2){
		if ((fp18 = fopen("vm1_data.out","w")) == NULL){
			printf("Can't open File\n");
			exit(1);
		}
		if ((fp19 = fopen("vm2_data.out","w")) == NULL){
			printf("Can't open File\n");
			exit(1);
		}
	}
	...
	for(var.beat=0; var.beat < beats; var.beat++){
		...
		for (j = 0; j< (var.m * var.BCL ); j++){
			...
			if(var.out_data_plus2){
				if (time == 0.0 || time < (beats-20)*var.BCL){
					if(j%(int)cut == 0){ // To reduce the recording data.
						vm1_data(fp18,time);
					}
				} else if(time == (beats-20)*var.BCL || time < (beats-10)*var.BCL){
					if(j%(int)cut == 0){ // To reduce the recording data.
						vm2_data(fp19,time);
					}
				}
			}
			if (time>= (beats-10)*var.BCL){
				if(j%(int)cut == 0){ // To reduce the recording data.
					vm_data(fp7,time);
				}
			}
